...
export function QuadroHorariosPage() {
    ...
    const [TurmasManha, setTurmasManha] = useState<Turma[]>([]);
    const [TurmasTarde, setTurmasTarde] = useState<Turma[]>([]);
    ...
    async function handleGetSchoolsData() {
        try {
            ...
            await data.turmas.map((turma: Turma) => {
                if (turma.turno === "MANHA" || turma.turno === "INTEGRAL") setTurmasManha(prev => [...prev, turma])
                if (turma.turno === "TARDE" || turma.turno === "INTEGRAL") setTurmasTarde(prev => [...prev, turma])
            })
                        ...
                        <table className="table-fixed border-collapse">
                            <thead>
                                ...
                                <tr className="bg-gray-100">
                                    ...
                                    {TurmasManha.map((turma: Turma) => (
                                        <th key={turma.id} className="w-40 border border-gray-300 p-1 text-center">
                                            <div className="text-xs font-bold text-gray-600">{turma.codigoTurma}</div>
                                            <div className="text-[10px] text-gray-400">{turma.idEcidade}</div>
                                        </th>
                                    ))}
                                </tr>
                            </thead>
