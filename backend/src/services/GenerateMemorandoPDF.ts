...
export async function GenerateMemorandoPDF(dados: Task) {
    ...
    const logoPrefeituraPath = path.join(__dirname, 'src', 'services', 'assets', 'prefeitura.jpg')
    ...
    const logoAssinaturaPath = path.join(__dirname, 'src', 'services', 'assets', 'assinatura.png')
    ...
    ListaFunc.forEach((servidor, index) => {
        ...
        memorandoFormat += `
            <div class="container">
                ...
                <div class="destino">
                    Destino: <strong>${servidor.entrada}</strong>
                </div>
                ...
                <div class="text-body">
                    Pelo presente, encaminho a V. Sª, o(a) funcionário(a) <strong>${servidor.servidor.toUpperCase()}</strong>, 
